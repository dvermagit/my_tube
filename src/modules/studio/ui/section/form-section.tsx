...
// import { z } from "zod";
// import { categories, videoUpdateSchema } from "@/db/schema";
...
// import { ThumbnailGenerateUploadModal } from "../components/thumbnail-generate-modal";
...
//                           variant="outline"
//                           type="button"
//                           className="rounded-full [&svg]:size3"
...
const FormSectionSuspense = ({ videoId }: FormSectionProps) => {
  ...
  // Fetch video and categories data
  const [video] = trpc.studio.getOne.useSuspenseQuery({ id: videoId });
  const [categories] = trpc.categories.getMany.useSuspenseQuery();

  const utils = trpc.useUtils();

  // Mutations for updating, deleting, generating, etc.
  const update = trpc.videos.update.useMutation({
    onSuccess: () => {
